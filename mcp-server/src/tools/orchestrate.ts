...
function buildOrchestrationGuide(task: string, complexity: Complexity): string {
  const guides: Record<Complexity, string> = {
...
export function registerOrchestrate(server: McpServer): void {
  server.registerTool(
    "orchestrate",
    {
      title: "오케스트레이션 가이드 반환",
      description:
        "태스크와 복잡도에 따른 에이전트 편성 가이드와 프롬프트 템플릿을 반환합니다.",
    ...
    async ({ task, complexity }) => {
      const level: Complexity = complexity ?? "medium";
