...
export function registerGetRules(server, novaRoot) {
    server.registerTool("get_rules", {
        title: "Nova 규칙 조회",
        description: "Nova 품질 게이트 규칙 전문을 반환합니다. section을 지정하면 해당 섹션(§1~§9)만 반환합니다.",
        inputSchema: z.object({
            section: z
                .string()
                .regex(/^\d+$/, "섹션 번호는 숫자만 허용됩니다")
                .optional()
                .describe("특정 섹션 번호 (예: '1', '2'). 미지정 시 전체 규칙 반환"),
        }),
    }, async ({ section }) => {
