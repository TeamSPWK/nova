...
state_changes = parse_state_md_diff()

if git_changes != ∅ and "NOVA-STATE.md" ∉ state_changes:
  → FAIL: "State Drift 의심 — 코드는 바뀌었으나 STATE 미갱신"
if state_changes.handoff.outputs ⊄ git_changes:
  → WARN: "handoff outputs에 명시된 파일이 git diff에 없음"
```
