...
2. [커맨드 상세](#커맨드-상세)
3. [전문가 에이전트](#전문가-에이전트)
4. [Orchestrator](#orchestrator-novaauto)
5. [MCP 서버](#mcp-서버)
...
다관점 수집(`/nova:ask` Mode A)을 사용하려면 AI API 키가 필요합니다:
...
> **API 키 없이도 다관점 수집 가능!** `/nova:ask`는 API 키 없이 에이전트 모드(Mode B)로 자동 전환됩니다.
...
Claude Code v2.1.98부터 Bash 도구의 보안 권한이 강화되었습니다. `/dev/tcp` 리다이렉트, 복합 명령어, 패턴 파일 읽기 등에서 추가 권한 프롬프트가 발생할 수 있습니다. Nova의 자동 검증(`/run`, `/check`)에서 Bash를 사용하는 경우 `bypassPermissions` 설정 또는 `allowedTools`에 해당 명령을 등록하세요.
...
| Design 있고 코드 커밋 있지만 검증 없음 | `/nova:check` |
| 검증 완료 | `/nova:review` |
| 모두 완료 | "다음 기능 시작 준비 완료" |

**출력 예시:**
```
추천: /nova:check docs/designs/auth.md src/
...
이후 흐름: /nova:check → /nova:review
```

---

### `/nova:setup` — 프로젝트 초기 설정

Nova 디렉토리 구조와 CLAUDE.md를 자동 생성합니다.

```bash
/nova:setup my-project            # 기본 (프로젝트명만)
/nova:setup                       # 대화형 — 정보를 물어봄
```
...
**기존 프로젝트에 도입할 때:**
```bash
/nova:setup --adopt my-project
```
→ 기존 CLAUDE.md를 수정하지 않고, 끝에 Nova 섹션만 추가합니다.

**모노레포 환경 (v3.7.0+):**

모노레포에서 `/nova:setup`을 실행하면 루트 감지 가드가 자동 동작합니다.
...
### `/nova:ask "질문"` — 멀티 AI 다관점 수집

3개 이상의 독립 관점에서 동시 질의하고 합의 수준을 자동 산출합니다.

```bash
/nova:ask "Next.js에서 서버 액션 vs API 라우트, 기본으로 어떤 걸 쓸까?"
/nova:ask "PostgreSQL vs MongoDB, 부동산 플랫폼에 적합한 건?"
/nova:ask --agent "API 키 없이 에이전트로 다관점 수집"
```
...
```bash
/nova:ask "기술적 질문"           # API 키가 있으면 자동으로 Mode A
```
...
```bash
/nova:ask --agent "질문"          # 명시적으로 에이전트 모드 사용
/nova:ask "질문"                  # API 키가 없으면 자동으로 Mode B
```
...
- **검증 계약**: `/nova:check`에서 검증할 테스트 가능한 성공 조건 목록
...
### `/nova:check 설계.md 코드/` — 통합 검증 (review + gap)

설계 정합성 검증(갭 탐지)과 `/nova:review`를 순서대로 실행하는 통합 커맨드입니다. 구현 완료 후 단일 명령으로 설계 정합성과 코드 품질을 동시에 점검합니다.

```bash
/nova:check docs/designs/auth.md src/
/nova:check                              # 최근 설계 문서 + 전체 src/ 자동 선택
/nova:check --strict docs/designs/auth.md src/   # 풀 검증 모드
```
...
**산출물:** `docs/verifications/{date}-check-{slug}.md`
...
Nova의 모든 검증 커맨드(`/nova:check`, `/nova:review`)는 다음 기준을 공유합니다.
...
- 불확실한 기술 선택은 `/nova:ask` 다관점 수집을 제안
...
`/nova:ask` 에이전트 모드(Mode B)에서는 Architect, Senior Dev, QA/Security 3개 관점을 병렬로 실행하여 다관점 수집합니다. 이는 전문가 에이전트의 핵심 활용 사례입니다.

---

## Orchestrator (`/nova:auto`)
...
# 멀티 프로젝트
/nova:auto "3개 프로젝트에 다크모드 통일"

# 설계만 (구현 전 확인용)
/nova:auto "새 대시보드 페이지 추가" --design-only

# QA 생략 (빠른 프로토타이핑)
/nova:auto "간단한 유틸 함수 추가" --skip-qa

# 엄격한 검증
/nova:auto "결제 모듈 리팩토링" --strict
```
...
`/nova:auto`: "만들어줘" → CPS 설계 → 구조화된 프롬프트 → 일관된 품질

핵심 가치: **구조화된 프롬프트가 자연어보다 낫다.** `/nova:auto`가 자연어를 CPS 설계로 변환하고, 그 설계서에서 Dev 에이전트에게 전달할 구체적인 프롬프트를 자동 생성한다.
...
3. /nova:ask "JWT vs 세션 기반 인증, SPA에 적합한 건?"
...
6. /nova:check docs/designs/user-auth.md src/
