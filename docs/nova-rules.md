...
| §17 Hook Safety Notes (v5.43.4+) | 신규 hook 추가/수정 시, `hooks/` 디렉토리 작업 시 |
...
> **Duration heuristic (옵션, soft signal)**: 1~2 파일 작업이라도 단일 함수 구현이 30분 이상 예상되면(복잡 알고리즘, 외부 시스템 연동, 마이그레이션 로직, 프로토콜 처리 등) Plan 승격을 권고한다. 강제 트리거가 아니며, 사용자 합의를 우선한다(`feedback_sprint_checkpoint.md`). 메모리 `feedback_nova_bypass_patterns.md`("소규모로 시작한 작업이 3파일+로 커질 때 승격 안 됨")의 사전 차단 시그널로 활용한다.
>
> 근거: Cline 공식 가이드(2026)의 "30분 이상 예상되는 작업은 step plan으로 분할" 권고. Nova의 파일 수 기준(3+)과 도메인 가중치(인증/DB/결제 +1)에 duration 신호를 보조 지표로 추가한다. 출처: https://docs.cline.bot/features/plan-and-act
...
- **커밋 전 하드 게이트 (Hard Gate)**: 구현 완료 → tsc/lint 통과 → Evaluator 실행 → PASS → 커밋 허용. Evaluator PASS 없이 커밋 시 `hooks/pre-commit-reminder.sh`가 `exit 2`로 차단한다. 사용자가 "Evaluator 돌렸니?"라고 물어보는 상황은 프로세스 실패다.
- **v5.48.1+ 자동 우회 메커니즘**:
  - **scope 필터**: staged 파일이 모두 doc-only(`README*`/`CHANGELOG*`/`LICENSE`/`.gitignore`/`docs/guides/**`/`dev/docs/**`/`.nova/**`/`docs/nova-meta.json`)면 자동 통과. **단** `docs/specs/**`/`docs/plans/**`/`docs/designs/**`/`docs/nova-rules.md`/`docs/eval-checklist.md`/`docs/nova-antipatterns.md`/`docs/proposals/**`는 코드와 동등 무게로 차단 유지.
  - **4시간 시간 윈도(파일 바인딩, v5.53.0+)**: `.nova/events.jsonl`의 `review_pass` 이벤트가 4시간(`NOVA_PASS_WINDOW_SEC` 환경변수로 오버라이드 가능, `0`이면 비활성화) 이내 **이고, 그 이벤트의 `files[].content_sha256`가 현재 staged 파일을 모두 커버**하면 자동 PASS. 무바인딩 `review_pass '{}'` 한 줄로 게이트를 통과시키던 self-attest 우회를 차단한다(측정 채널 ≠ 통과 채널). sha 소스는 staged blob(`git show :<path>`)이라 리뷰 후 내용을 바꾸면 재검증을 강제한다. 리뷰 측은 `scripts/lib/build-files-payload.sh`로 동일 sha를 산출한다. 자정 경계 강제 재검증 페인을 제거하며 단일 진실원(events.jsonl) 정책과 정합.
- **명시적 우회**: `--emergency` 플래그(프로덕션 장애 긴급 수정) 또는 `NOVA_EMERGENCY=1`. CONFLICT 상태는 우회 불가.
- **다른 플러그인과 공존 (v5.53.0+)**: OMC 등 세션 소유형 오케스트레이션 플러그인과 같이 쓸 때 `NOVA_COEXIST=1`로 **커밋 게이트만 남기고** 나머지 훅(규칙 주입·per-tool 관찰성·stop·pre-compact·pre-edit)을 no-op한다. **게이트는 유지되므로 우회가 아니다.** 토글: `bash scripts/nova-coexist.sh on|off|status`. 기본값(미설정)은 full Nova → 기존 사용자/팀원 영향 0(완전 opt-in). 상세: `docs/guides/coexist.md`.
...
> **팀원 종료 의무 (v5.47.8+, idle ≠ 종료)**:
> TeamCreate 또는 `Agent({team_name: ..., name: ...})`로 teammate를 spawn한 lead는 작업 완료 직후 **반드시** 각 teammate에게 `SendMessage({type:"shutdown_request"})`를 발송한다.
> Claude Code의 `idle_notification`은 종료 신호가 **아니며**, lead가 shutdown_request를 보내고 teammate가 approve할 때까지 process가 살아 있어 tmux pane·세션 비용을 점유한다.
> 발송은 작업 보고와 **동일 turn**에 한다(다음 turn으로 미루면 망각). 모든 teammate의 shutdown 응답 확인 후 필요 시 `TeamDelete`로 팀 디렉토리를 정리한다. `TeamDelete`는 활성 멤버가 남아 있으면 실패하므로 shutdown 응답을 먼저 확인한다.
> 코드화 위치: `.claude/skills/orchestrator/SKILL.md` Phase 7 + `dev/commands/evolve.md` Phase 1 (Nova 개발자 전용) + `hooks/audit-teammates.sh` (Stop·SessionStart 좀비 audit, v5.47.9+ · §B7 antipattern). (참조: MEMORY `feedback_shutdown_idle_agents.md`, v5.47.x 자기-발견 갭)

## §3. 검증 기준

- **기능**: 요청한 것이 실제로 동작하는가? (CLAUDE.md 또는 NOVA-STATE.md의 요구사항 원문과 대조)
- **데이터 관통**: 입력 → 저장 → 로드 → 표시까지 완전한가? 트리거 이후 사용자에게 실제 전달되는가?
- **사용자 가이드 동기화** (v5.25.0+): 신규 `scripts/*.sh` / `.github/workflows/*.yml` / 외부 호출 진입점이면 `docs/guides/<주제>.md`를 동시 작성한다. spec(개발자 계약)과 guide(사용자 절차)는 별개 산출물 — guide 누락은 "데이터 관통" 위반. 회귀 가드 자동 검증.
...
- **자기 보안 진단** (v5.22.0+, Nova 개발자 전용 — `dev/commands/audit-self.md`): Nova 플러그인 자기 코드(plugin.json/hooks/agents/skills/commands)는 정적 보안 룰셋(`docs/security-rules.md`)으로 검사. 플러그인 사용자에게는 노출되지 않음.
...
- **데이터 모델 분리 (v5.44.0+)**: NOVA-STATE.md = **현재 상태 스냅샷** (Current/Goal/Phase/Refs/Risks — 사람·AI가 손편집, 본질적으로 작음). `.nova/events.jsonl` = **활동 시계열 단일 진실원** (rotation 자체 관리). NOVA-STATE.md의 Recent Activity / Recently Done 표는 v3 marker가 있으면 `scripts/registry-render-state.sh`가 자동 갱신, 없으면 비어있음 (활동은 `/nova:status` HTML 대시보드로).
- **AI는 시계열 표를 손편집하지 않는다.** 8개 진입점(plan/design/deepplan/run/auto/review/check/ux-audit)이 끝나면 events.jsonl 기록(자동) + 본문 스냅샷 갱신만 수행. 트림 의무 없음. (개발자 전용 `dev/commands/evolve.md`도 동일 모델을 따른다.)
- **Known Gaps 필수**: 검증 후 발견된 미커버 영역을 NOVA-STATE.md(Risks/Known Gaps 섹션)에 기록한다. "ALL PASS"만 기록하면 과신을 유도한다.

**즉시 업데이트 트리거** (본문 스냅샷만; 시계열은 자동):
- 블로커 발생/해소 → Risks 섹션
- 검증 FAIL 수신 → Risks 섹션

**커밋 전 일괄 갱신**: Phase 전환, 새 Refs는 커밋 전에 일괄 반영한다. 작업 중 관성적 갱신을 줄여 흐름을 유지.

**상태-수준 ≠ 세션-수준 (v5.21.0+, v5.44.0 재정의)**: 상태-수준 모델(events.jsonl 단일 진실원 + 본문 스냅샷)은 데이터 구조 분리지 Claude 컨텍스트 창 압축이 아니다. 세션-수준 압축은 `skills/strategic-compact/SKILL.md` (마일스톤 직후 / 토큰 70%+ / 무관 작업 전환). 둘을 같은 것으로 혼동하면 토큰 압박 해소 불가.

**상태 진실원 계약 (v5.45.0+)**:
- registry(`.nova/work-items`) = **status 단일 진실** — WI가 "done"인지는 registry가 결정한다.
- git log = **완료 단일 진실** — 실제 작업이 완료됐는지는 커밋이 증거다.
- NOVA-STATE.md prose = status를 갖지 않는 **비공식 스냅샷** (Goal·관찰·서사). 충돌 시 prose는 비권위 — 진다.
...
- **코드 실행 권한을 부여하는 설정 파일은 환경 전환 외에도 임의 코드 실행 위험이 있다** — `.npmrc`/`.yarnrc*`/`bunfig.toml`/`.bazelrc`/`.pre-commit-config.yaml`/`.devcontainer/`/shell 시작 파일(`.zshenv`/`.zlogin`/`.bash_login`)/`~/.config/git/`은 다음 셸·빌드·커밋 시점에 실행되므로, 명시적 필요·확인 없이 생성·수정하지 않는다. (Claude Code v2.1.160이 동일 위험을 acceptEdits 쓰기-확인 가드로 강화 — Nova §9 원칙의 수렴 검증.)
...
**Top-level 필드 확장 (v5.43.5+, M-1, schema_version 3 유지)**: 모든 이벤트에 nullable `agent_id` / `parent_agent_id` 자동 캡처. `$CLAUDE_AGENT_ID` / `$CLAUDE_PARENT_AGENT_ID` 환경변수 노출 시 채워짐 (CC v2.1.139+ subagent header `x-claude-code-agent-id`와 의미적으로 일치). 미노출 시 `null` — forgiving reader 원칙으로 분석기가 무시 가능, 기존 v3 레코드와 호환되므로 schema_version은 3 유지. **현재 CC가 hook subprocess에 두 env를 직접 주입하는지 공식 사양 미확인** — 노출되면 Generator-Evaluator 체인 측정 가능, 미노출이어도 always-null로 시스템 안전. 실제 서브에이전트 환경에서 필드 채워짐 여부 확인 권장.
...
**MCP 카운트 자동 알림 (v5.22.1+)**: `hooks/session-start.sh`가 `claude mcp list`를 1시간 캐시(`.nova/mcp-count.cache`)로 호출 → MCP 서버 **>10개일 때만** standard/strict 프로파일에 ⚠️ 1줄 경보 주입. 정상 카운트(≤10)는 노출 없음(시그널 강도 보존). lean 프로파일은 ≤1200자 예산 보호로 미주입. 캐시 미스 시 백그라운드 갱신 — 본 세션 응답 시간 영향 0.
...
| Gap detection rate | evaluator_verdict=FAIL 중 같은 orchestration_id 내 이후 sprint_completed(verdict=PASS) 또는 phase_transition(to_status=completed)로 종결된 비율 |
| Rule evolution rate | events.jsonl `evolve_decision` 이벤트 중 `extra.decision==accept` 비율 (measurement-spec.md §6 재정의 — rules-changelog grep 폐기) |
...
**In-situ 검증 권장**:
`tests/test-scripts.sh`는 stdin 모의 기반. 실제 Claude Code 런타임이 precheck-tool을 호출하는지 확인하려면 Nova 개발자 전용 `dev/skills/field-test/`로 격리된 worktree에서 실 플러그인 E2E 테스트 권장 (사용자 플러그인에는 미포함).
...
## §17. Hook Safety Notes (CC v2.1.139~143 정합, v5.43.4+)

Claude Code v2.1.139~143에서 hook·세션 모델이 갱신됐다. Nova hook 작성/유지보수 시 다음 안전 메모를 참조한다. on-demand 로드 (session-start.sh 미주입).

### Stop hook 블록 금지 (v2.1.143 cap=8)

- Stop hook이 반복 블록할 경우 v2.1.143은 **8회 연속 후 강제 turn 종료**. 환경변수 `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`로 조정 가능.
- Nova `hooks/stop-event.sh`는 **항상 `exit 0`** (observability 기록 only). 영향 없음.
- **회귀 가드**: `tests/test-scripts.sh`가 `stop-event.sh`에 `exit 2` 또는 `"block"` decision 패턴 등장 시 FAIL. 신규 Stop hook 추가 시 동일 패턴 유지.
- 출처: https://code.claude.com/docs/en/changelog (v2.1.143)

### PostToolUse `continueOnBlock` (v2.1.139+)

- PostToolUse 훅에 `continueOnBlock: true` 옵션 추가. 훅이 거부 사유를 반환하면 Claude에게 직접 피드백.
- Nova evaluator는 **별도 agent 호출 기반**이라 본 옵션 즉시 적용 X. 향후 secret leak 등 패턴 감지 시 자동 피드백 루프 후보로 검토.
- 출처: https://code.claude.com/docs/en/changelog (v2.1.139)

### 백그라운드 세션 영속성 (v2.1.143+)

- 백그라운드 세션이 idle 후 wake할 때 model + effort level 유지. `/bg` 커맨드가 `--mcp-config`, `--settings`, `--add-dir`, `--plugin-dir`, `--strict-mcp-config`, `--fallback-model`, `--allow-dangerously-skip-permissions` 모두 보존.
- Nova가 향후 백그라운드 세션 활용 시 model/effort 명시 재지정 불요.
- 출처: https://code.claude.com/docs/en/changelog (v2.1.143)

### Hook exec form (`args: string[]`, v2.1.139+)

- hook 정의에 `args: string[]` 필드 추가 — 셸 없이 직접 spawn. 셸 메타문자 인젝션 위험 차단.
- Nova `hooks/hooks.json` 모든 hook은 현재 `command: "bash ..."` shell form. 외부 입력 직접 보간 없음 → 보안 위험 낮음.
- **권장**: 신규 hook 추가 시 인자 입력이 외부 데이터(예: 사용자 메시지 일부)일 경우 `args: string[]` exec form 사용.
- 출처: https://code.claude.com/docs/en/changelog (v2.1.139)

### Desktop notification opt-in (`NOVA_DESKTOP_NOTIFY=1`, v5.43.5+, M-2)

- CC v2.1.141부터 hook JSON 출력에 `terminalSequence` 필드를 emit하면 controlling terminal 없어도 bell + window title 변경.
- Nova `hooks/stop-event.sh`는 `NOVA_DESKTOP_NOTIFY=1` 환경변수 + `.nova/events.jsonl` 마지막 5 이벤트에 `commit_blocked` 또는 `evaluator_verdict=FAIL`이 있을 때만 `{"terminalSequence":"<BEL>]0;Nova: BLOCKED<BEL>"}`을 stdout으로 emit.
- 기본은 비활성(미설정). 사용자가 백그라운드 작업 중 블록 상태 즉시 인지하려면 `export NOVA_DESKTOP_NOTIFY=1`.
