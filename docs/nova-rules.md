...
> 코드화 위치: `skills/orchestrator/SKILL.md` Phase 7 + `commands/evolve.md` Phase 1 + `hooks/audit-teammates.sh` (Stop·SessionStart 좀비 audit, v5.47.9+ · §B7 antipattern). (참조: MEMORY `feedback_shutdown_idle_agents.md`, v5.47.x 자기-발견 갭)
...
- **AI는 시계열 표를 손편집하지 않는다.** 9개 진입점(plan/design/deepplan/run/auto/review/check/ux-audit/evolve)이 끝나면 events.jsonl 기록(자동) + 본문 스냅샷 갱신만 수행. 트림 의무 없음.
