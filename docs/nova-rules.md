...
- **v5.48.1+ 자동 우회 메커니즘**:
  ...
  - **4시간 시간 윈도**: `.nova/events.jsonl`의 가장 최근 `review_pass` 이벤트가 4시간(`NOVA_PASS_WINDOW_SEC` 환경변수로 오버라이드 가능, `0`이면 비활성화) 이내면 자동 PASS. 자정 경계 강제 재검증 페인을 제거하며 단일 진실원(events.jsonl) 정책과 정합.
