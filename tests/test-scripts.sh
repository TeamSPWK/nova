...
#    - soft target 2200 bytes: 부채 인정 (v5.22.1 MCP 알림 1993 → v5.31.0 §15 Memory routing 2152 정착)
...
assert "session-start 출력 크기 soft target 2200 bytes 이하 ($SESSION_SIZE)" \
  "[ $SESSION_SIZE -le 2200 ]"
...
# session-start.sh 크기 여전히 soft 2200 이하 (회귀, 부채 인정)
assert "Sprint 1 회귀: session-start 출력 여전히 soft 2200 bytes 이하" \
  "[ \$(bash '$ROOT_DIR/hooks/session-start.sh' | wc -c | tr -d ' ') -le 2200 ]"
...
# session-start 크기 회귀
assert "Sprint 2a 회귀: session-start 여전히 soft 2200 이하" \
  "[ \$(bash '$ROOT_DIR/hooks/session-start.sh' | wc -c | tr -d ' ') -le 2200 ]"
...
# session-start 크기 회귀
assert "Sprint 2b 회귀: session-start 여전히 soft 2200 이하" \
  "[ \$(bash '$ROOT_DIR/hooks/session-start.sh' | wc -c | tr -d ' ') -le 2200 ]"
