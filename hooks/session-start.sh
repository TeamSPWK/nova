...
if [ "$NOVA_PROFILE" = "lean" ]; then
  ADDITIONAL_CONTEXT="# Nova Engineering (lean)\n\nNova lean 모드 — 핵심 규칙만 적용. antipatterns 체크 스킵. pre-edit CPS 경고 스킵.\n\n## 규칙 (lean 핵심)\n\n1. **복잡도**: 간단(1~2)→바로. 보통(3~7)→Plan. 복잡(8+)→Plan→Design→스프린트. 자가 완화 금지.\n2. **검증 + 하드 게이트**: 검증은 독립 서브에이전트. 커밋 전 Evaluator PASS 필수. PASS 없이 커밋 시 exit 2 차단(v5.48.1+ doc-only/4h 윈도 자동 통과, --emergency 우회).\n3. **실행 검증**: 코드 존재 ≠ 동작. 빌드+테스트+curl.\n\n## Nova 커맨드\n\n/nova:plan · /nova:deepplan · /nova:design · /nova:review · /nova:check · /nova:ask · /nova:run · /nova:setup · /nova:next · /nova:status · /nova:scan · /nova:auto · /nova:ux-audit · /nova:worktree-setup · /nova:claude-md · /nova:migrate-state · /nova:checkpoint\n\n## Always-On (MUST)\n\n1. 커밋 전 /nova:review --fast.\n2. NOVA-STATE.md는 본문 스냅샷만 손편집 — 시계열은 events.jsonl 자동(v5.44.0+).\n3. UI 변경 시 G1+G3 시각 게이트 발화 (lean도 적용 — §14).\n4. 팀 spawn → teammate shutdown_request 의무 (idle≠종료, §2)."

# strict: standard + antipatterns 요약 추가
elif [ "$NOVA_PROFILE" = "strict" ]; then
  ADDITIONAL_CONTEXT="# Nova Engineering (strict)\n\nNova 자동 적용 규칙 — 품질 실행 계약. 상세는 docs/nova-rules.md 및 관련 커맨드가 on-demand 로드.\n\n## 규칙 (핵심)\n\n1. **복잡도**: 간단(1~2)→바로. 보통(3~7)→Plan. 복잡(8+)→Plan→Design→스프린트. 자가 완화 금지. 파일 수 초과 시 즉시 Plan 승격. duration heuristic(옵션): 1~2파일도 30분+ 예상이면 Plan 권고.\n2. **검증 + 하드 게이트**: 검증은 **독립 서브에이전트**. 커밋 전 Evaluator PASS 필수. exit 2 차단(v5.48.1+ doc-only/4h 윈도 자동 통과, --emergency 예외).\n3. **실행 검증**: 코드 존재 ≠ 동작. 빌드+테스트+curl. 환경 변경 3단계.\n4. **블로커**: Auto/Soft/Hard. 불확실=Hard.\n5. **환경 안전**: 설정 파일 직접 수정 금지.\n\n## Antipatterns — docs/nova-antipatterns.md\n\n§A1 복잡도 자가 완화 · §A3 Evaluator 후순위 · §B1 Evaluator 건너뛰고 커밋 · §B2 CPS 없이 구현 · §B3 세션 상태 갱신 생략 (전체 13개)\n\n## Nova 커맨드\n\n/nova:plan · /nova:deepplan · /nova:design · /nova:review · /nova:check · /nova:ask · /nova:run · /nova:setup · /nova:next · /nova:status · /nova:scan · /nova:auto · /nova:ux-audit · /nova:worktree-setup · /nova:claude-md · /nova:migrate-state · /nova:checkpoint\n\n## Always-On\n\n1. 3파일+ 변경 시 Plan. 2. Evaluator 독립 서브에이전트. 3. 커밋 전 /nova:review --fast. 4. NOVA-STATE.md는 본문 스냅샷만 손편집 — 시계열은 events.jsonl 자동(v5.44.0+). 진행 중 작업은 prose 아닌 work-item으로(§8 계약). 5. UI 변경 시 G1+G3 시각 게이트 발화. 6. §15 Memory 라우팅: 프로젝트 규칙은 개인 memory 금지 → CLAUDE.md/AGENTS.md/\`.claude/rules/\`. 7. 팀 spawn → teammate shutdown_request 의무 (idle≠종료, §2)."

# standard (기본): 현재와 동일
else
  ADDITIONAL_CONTEXT="# Nova Engineering\n\nNova 자동 적용 규칙 — 품질 실행 계약. 상세는 docs/nova-rules.md 및 관련 커맨드가 on-demand 로드. 프로젝트 \`.claude/rules/\`가 있으면 Nova보다 우선.\n\n## 규칙 (핵심)\n\n1. **복잡도**: 간단(1~2)→바로. 보통(3~7)→Plan. 복잡(8+)→Plan→Design→스프린트. 인증/DB/결제 +1. 자가 완화 금지. 파일 수 초과 시 즉시 Plan 승격. duration heuristic(옵션): 1~2파일도 30분+ 예상이면 Plan 권고.\n2. **검증 + 하드 게이트**: 검증은 **독립 서브에이전트**(별도 spawn=창 분리). 메인 재확인은 독립 아님. 커밋 전 tsc/lint→Evaluator→PASS→커밋. PASS 없이 커밋 시 exit 2 차단(v5.48.1+ doc-only/4h 윈도 자동 통과, --emergency 예외). tmux pane: TeamCreate→Agent(name+team_name+run_in_background:true).\n3. **실행 검증**: 코드 존재 ≠ 동작. 빌드+테스트+curl. 환경 변경 3단계(현재→변경→반영).\n4. **블로커**: Auto/Soft/Hard. 불확실=Hard. 2회 실패 시 강제 분류.\n5. **환경 안전**: 설정 파일 직접 수정 금지. 환경변수/CLI 플래그.\n\n## Nova 커맨드\n\n/nova:plan · /nova:deepplan · /nova:design · /nova:review · /nova:check · /nova:ask · /nova:run · /nova:setup · /nova:next · /nova:status · /nova:scan · /nova:auto · /nova:ux-audit · /nova:worktree-setup · /nova:claude-md · /nova:migrate-state · /nova:checkpoint\n\n## Always-On (MUST)\n\n1. 모든 코드 변경에 자동 규칙. 2. 3파일+ 변경 시 Plan. 3. 구현 완료 시 Evaluator 독립 서브에이전트. 4. 커밋 전 /nova:review --fast. 5. NOVA-STATE.md는 본문 스냅샷만 손편집 — 시계열은 events.jsonl 자동(v5.44.0+). 진행 중 작업은 prose 아닌 work-item으로(§8 계약). 6. 블로커 즉시 알림. 7. UI 변경 시 G1+G3 시각 게이트. 8. §15 Memory 라우팅: 프로젝트 규칙은 개인 memory 금지 → CLAUDE.md/AGENTS.md/\`.claude/rules/\`. 9. 팀 spawn → teammate shutdown_request 의무 (idle≠종료, §2)."
fi
...
# ── measurement Phase 1 4주 미갱신 리마인더 (Sprint 3, measurement-spec.md §4 알고리즘 4) ──
  ...
  # NOVA_MEASUREMENT_BASELINE=1 시 skip — 사이즈 회귀 측정 격리 (release-time advisory noise 제거, v5.48.0+).
  if [[ "${NOVA_MEASUREMENT_BASELINE:-0}" != "1" ]] && [[ -f .nova/impl-tracker.json ]] && command -v jq >/dev/null 2>&1; then
...
# ── S3 session-start advisory: STATE 드리프트 감지 시 1줄 권장 ──
# timeout 2초 — 폴백: gtimeout → perl alarm → 무제한(최후). 실패 시 silent skip.
# 경량 예산: drift 있을 때만 1줄 추가. 초과 금지.
_DRIFT_ADVISORY=""
_nova_reconcile_bin="${NOVA_ROOT}/scripts/reconcile-state.sh"
# NOVA_MEASUREMENT_BASELINE=1 시 skip — 사이즈 회귀 측정 격리 (v5.48.0+).
if [ "${NOVA_MEASUREMENT_BASELINE:-0}" != "1" ] && [ -f "$_nova_reconcile_bin" ] && [ "$NOVA_PROFILE" != "lean" ]; then
  _adv_out=""
  if command -v timeout >/dev/null 2>&1; then
    _adv_out=$(timeout 2 bash "$_nova_reconcile_bin" --jsonl 2>/dev/null) || true
  elif command -v gtimeout >/dev/null 2>&1; then
    _adv_out=$(gtimeout 2 bash "$_nova_reconcile_bin" --jsonl 2>/dev/null) || true
  elif command -v perl >/dev/null 2>&1; then
    _adv_out=$(perl -e 'alarm 2; exec @ARGV' bash "$_nova_reconcile_bin" --jsonl 2>/dev/null) || true
  else
    _adv_out=$(bash "$_nova_reconcile_bin" --jsonl 2>/dev/null) || true
  fi
  if [ -n "$_adv_out" ] && echo "$_adv_out" | jq -e . >/dev/null 2>&1; then
    _adv_s=$(echo "$_adv_out" | jq '(.counts.suspect_explicit // 0) + (.counts.suspect_fuzzy // 0)' 2>/dev/null || echo "0")
    _adv_u=$(echo "$_adv_out" | jq '(.counts.untracked // 0)' 2>/dev/null || echo "0")
    _adv_total=$(( _adv_s + _adv_u ))
    if [ "$_adv_total" -ge 1 ]; then
      _DRIFT_ADVISORY="\n\nSTATE 드리프트 ${_adv_total}건 — \`/nova:checkpoint\` 권장"
    fi
  fi
fi
