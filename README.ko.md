...
[![Version](https://img.shields.io/badge/version-5.55.1-blue)](https://github.com/TeamSPWK/nova/releases)
...
설치 후 Codex를 재시작한다. 자세한 옵션과 장애 대응은 [Codex 플러그인 설치 가이드](docs/guides/codex-plugins.md)를 참고한다.
...
`CLAUDE.md`, `AGENTS.md`, rules, settings, hooks의 역할을 나눠 정리하려면 `/nova:claude-md`만 입력한다. 짧은 안내 후 수정 없이 현재 레포를 감사한다. 자세한 기준은 [Agent Instruction Guide](docs/guides/claude-md.md)를 참고한다.
...
> 세션 간 컨텍스트 체인은 **append-only JSONL + 텍스트 NOVA-STATE.md** — 외부 의존성 0 (vector DB 없음, embedding API 없음). 외부 대안과 트레이드오프 비교: [Nova vs claude-mem vs Continuous-Claude-v3](docs/comparison/context-chain-vs-external.md).
...
Nova는 [Codex](https://github.com/openai/codex) 사용자를 위한 별도 매니페스트를 제공한다. Phase 1에서는 스킬과 MCP를 사용할 수 있다.

추천 플러그인 세트(Browser Use, Documents, Spreadsheets, Presentations, Nova)는 한 줄로 설치할 수 있다. 자세한 절차와 장애 대응은 [Codex 플러그인 설치 가이드](docs/guides/codex-plugins.md)를 참고한다.
...
- [Agent Instruction Guide](docs/guides/claude-md.md) — `/nova:claude-md`가 CLAUDE.md, AGENTS.md, rules, settings, hooks를 나누는 기준
- [Skill Governance 가이드](docs/guides/skill-governance.md) — `skillOverrides`로 Nova 스킬 선택적 비활성화 (CC v2.1.126+)
- [State Drift Reconciliation 가이드](docs/guides/state-drift-reconciliation.md) — NOVA-STATE.md prose·git log·registry 3-way 드리프트 탐지 및 해소
