...
Plan 문서 저장 직후, 메인 에이전트가 **registry-write.sh create**를 호출하여 work-item을 발급한다 (status=proposed). NOVA-STATE.md 직접 갱신 금지 — marker 영역은 `registry-render-state.sh`가 자동 렌더한다.

```bash
WI_ID=$(bash "$NOVA_PLUGIN_PATH/scripts/registry-write.sh" create "<Plan title>" \
  --priority=<critical|high|medium|low> \
  --source-doc=docs/plans/<slug>.md)
echo "발급된 work-item: $WI_ID"
```

호출 권한: 메인 에이전트만 (Generator-Evaluator 원칙 + 단일 쓰기 경로). sub-agent는 직접 호출 금지.
