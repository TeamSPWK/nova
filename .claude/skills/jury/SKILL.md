...
## 모드 3: 보안 진단 (v5.23.0+, ECC §P2-3 흡수, Nova 개발자 전용)

`dev/commands/audit-self.md --jury`에서 호출된다 — Nova 자기 코드 대상이므로 플러그인 사용자에게는 노출되지 않는다. 호출 컨텍스트에 `mode: audit`, `target: nova-self-codebase`, `rules_doc: docs/security-rules.md`, `scan_targets`, `exclusion_list`가 포함된다.
