...
세션-수준 컨텍스트 압축 시점을 판단한다. NOVA-STATE 본문 스냅샷 + events.jsonl 분리(v5.44.0+ 상태-수준 모델)와 짝을 이루는 *세션-수준* (Claude Code 컨텍스트 창 자체)의 `/clear`·`/compact` 적기를 안내한다. Nova 5기둥 중 **맥락 기둥**의 마지막 한 조각 — 진단(P0-1 컨텍스트 로스트 4원인 카탈로그)에서 자동 안내까지 잇는 출구.
...
- **예방**: 본 스킬 (세션-수준) + events.jsonl 단일 진실원 + v3 marker 자동 렌더 (상태-수준, v5.44.0+) 이중 압축
